...
get_data = function(seed){
  read.csv(get_path(paste0("data_generation/different_confounding_levels/data/data_obs_large_conf_", conf, "_"), seed))
}

get_true_mixing_matrix = function(seed){
  read.csv(get_path(paste0("data_generation/different_confounding_levels/data/true_mixing_large_conf_", conf, "_"), seed))
}
...
table(apply(p_values_iv,1, which.max))
table(apply(p_values_indp_undcond, 1,function(x) sum(x <.01)))
