...
def create_ICA_output(estimator, dgp):
    ...
    for cn in level_of_confounding:
        ...
        for i in tqdm(range(B)):
            ...
            pd.DataFrame(data_generator.mixing_matrix_observed).to_csv(f"different_confounding_levels/data/true_mixing_large_conf_{cn}_{i}.csv", index = False)
            pd.DataFrame(data_generator.signals).to_csv(f"different_confounding_levels/data/true_signals_large_conf_{cn}_{i}.csv", index = False)
            pd.DataFrame(data_generator.data_observed).to_csv(f"different_confounding_levels/data/data_obs_large_conf_{cn}_{i}.csv", index = False)
            ...
            pd.DataFrame(est.A).to_csv(f"different_confounding_levels/data/estimated_mixing_CausalVarEM_large_conf_{cn}_{i}.csv", index = False)
            pd.DataFrame(est.Signals).to_csv(f"different_confounding_levels/data/estimated_signals_CausalVarEM_large_conf_{cn}_{i}.csv", index = False)
