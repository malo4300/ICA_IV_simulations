...
def create_ICA_output(estimator, dgp):
    ...
    for covs in number_of_covariates:
        ...
        for i in tqdm(range(B)):
            ...
            pd.DataFrame(data_generator.mixing_matrix_observed).to_csv(f"different_numbers_of_covariates/data/true_mixing_covs_{covs}_{i}.csv", index = False)
            pd.DataFrame(data_generator.signals).to_csv(f"different_numbers_of_covariates/data/true_signals_covs_{covs}_{i}.csv", index = False)
            pd.DataFrame(data_generator.data_observed).to_csv(f"different_numbers_of_covariates/data/data_obs_covs_{covs}_{i}.csv", index = False)
            ...
            pd.DataFrame(est.A).to_csv(f"different_numbers_of_covariates/data/estimated_mixing_CausalVarEM_covs_{covs}_{i}.csv", index = False)
            pd.DataFrame(est.Signals).to_csv(f"different_numbers_of_covariates/data/estimated_signals_CausalVarEM_covs_{covs}_{i}.csv", index = False)

            est = estimator(update_sigma=False, true_A=None, max_iter = iter,
                            random_seed= i,  mode = "VarEM",
                            init_range = [-3,3])
            est.fit(data_generator.data_observed,J, noise_params= {"mean" : 0, "std" : 1}, progress_bar=False)
            pd.DataFrame(est.A).to_csv(f"different_numbers_of_covariates/data/estimated_mixing_VarEM_covs_{covs}_{i}.csv", index = False)
            pd.DataFrame(est.Signals).to_csv(f"different_numbers_of_covariates/data/estimated_signals_VarEM_covs_{covs}_{i}.csv", index = False)
...
def calculate_p_values(conditional_function, unconditional_function):
    ...
    for cov in number_of_covariates:
        ...
        for i in tqdm(range(B)):
            data = pd.read_csv(f"different_numbers_of_covariates/data/data_obs_covs_{covs}_{i}.csv", header=0).values
            true_signals = pd.read_csv(f"different_numbers_of_covariates/data/true_signals_covs_{covs}_{i}.csv", header=0).values
        ...
        pd.DataFrame(p_values_conditional_true_signals).reset_index().to_csv(f"different_numbers_of_covariates/p_values_conditional_true_sources_covs_{covs}.csv", header=False, index = False)
        pd.DataFrame(p_values_unconditional_true_signals).reset_index().to_csv(f"different_numbers_of_covariates/p_values_unconditional_true_sources_covs_{covs}.csv", header=False, index = False)